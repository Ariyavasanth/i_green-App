  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (type == TransactionType.salesOrder) {
      return LayoutBuilder(
        builder: (context, constraints) {
          if (constraints.maxWidth < AppBreakpoints.tablet) {
            return SalesOrderMobileView(onAdd: () => context.push('/$path/new'));
          }
...
class _InvoiceMobileViewState extends ConsumerState<_InvoiceMobileView> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ref.watch(transactionsProvider(TransactionType.invoice)).when(
      loading: loading,
      error: error,
      data: (all) {
              ...
              if (shown.isEmpty)
                ...
                SliverPadding(
                  ...
                  sliver: SliverList.separated(
                    itemCount: shown.length + (shown.length < rows.length ? 1 : 0),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _InvoiceCard(
                        row: shown[index],
                        onAction: (action) => _handleAction(shown[index], action),
                      );
                    },
                  ),
                ),
  ...
  Widget _summary(List<SalesTransaction> rows) {
    ...
    final dueToday = unpaid.where((r) => r.dueDate != null && DateUtils.isSameDay(r.dueDate, now)).fold<double>(0, (s, r) => s + r.amount);
    final within30 = unpaid.where((r) => r.dueDate != null && r.dueDate!.isAfter(now) && r.dueDate!.isBefore(now.add(const Duration(days: 31)))).fold<double>(0, (s, r) => s + r.amount);
    final overdue = unpaid.where((r) => r.dueDate != null && r.dueDate!.isBefore(DateTime(now.year, now.month, now.day))).fold<double>(0, (s, r) => s + r.amount);
    return Card(
      margin: EdgeInsets.zero,
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Payment Summary', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
          const SizedBox(height: 14),
          Text(money.format(outstanding), style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: AppColors.primary)),
          const Text('Total Outstanding Receivables', style: TextStyle(color: AppColors.textSecondary)),
          const Divider(height: 24),
          Row(children: [
            Expanded(child: _SummaryValue('Due Today', money.format(dueToday))),
            Expanded(child: _SummaryValue('Due in 30 Days', money.format(within30))),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _SummaryValue('Overdue', money.format(overdue), warning: overdue > 0)),
  ...
  Widget _controls() => Column(children: [
    SearchBar(
      ...
      trailing: _search.text.isEmpty ? null : [IconButton(onPressed: () { _search.clear(); setState(() {}); }, icon: const Icon(Icons.close))],
    ...
    Row(children: [
      Expanded(
        child: DropdownButtonFormField<String>(
          ...
          decoration: const InputDecoration(prefixIcon: Icon(Icons.filter_list), labelText: 'Status'),
          items: const ['All', 'Draft', 'Sent', 'Paid', 'Overdue'].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
          onChanged: (v) => setState(() { _status = v ?? 'All'; _visibleCount = 10; }),
        ),
      ),
      ...
      Expanded(
        child: DropdownButtonFormField<_InvoiceSort>(
          initialValue: _sort,
          decoration: const InputDecoration(prefixIcon: Icon(Icons.sort), labelText: 'Sort'),
          items: const [
            DropdownMenuItem(value: _InvoiceSort.newest, child: Text('Newest')),
            DropdownMenuItem(value: _InvoiceSort.dueDate, child: Text('Due date')),
            DropdownMenuItem(value: _InvoiceSort.amountHigh, child: Text('Amount')),
            DropdownMenuItem(value: _InvoiceSort.customer, child: Text('Customer')),
          ],
  ...
  List<SalesTransaction> _filtered(List<SalesTransaction> all) {
    ...
    final rows = all.where((r) {
      final matchesText = '${r.number} ${r.customer} ${r.referenceNumber}'.toLowerCase().contains(q);
      final matchesStatus = _status == 'All' || _effectiveStatus(r).toLowerCase() == _status.toLowerCase();
      return matchesText && matchesStatus;
    }).toList();
    rows.sort((a, b) => switch (_sort) {
      _InvoiceSort.newest => b.date.compareTo(a.date),
      _InvoiceSort.dueDate => (a.dueDate ?? DateTime(9999)).compareTo(b.dueDate ?? DateTime(9999)),
      _InvoiceSort.amountHigh => b.amount.compareTo(a.amount),
      _InvoiceSort.customer => a.customer.toLowerCase().compareTo(b.customer.toLowerCase()),
    });
    return rows;
  }
  ...
  Future<void> _handleAction(SalesTransaction row, String action) async {
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$action is not available in the current API.')));
  }
}

String _effectiveStatus(SalesTransaction row) {
  if (row.status.toLowerCase() != 'paid' && row.dueDate?.isBefore(DateTime.now()) == true) return 'Overdue';
  final value = row.status.toLowerCase();
  return value.isEmpty ? 'Draft' : '${value[0].toUpperCase()}${value.substring(1)}';
}
...
class _SummaryValue extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w700, color: warning ? Colors.red.shade700 : null)),
...
class _InvoiceCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    final statusColor = status == 'Paid' ? AppColors.primary : status == 'Overdue' ? Colors.red.shade700 : AppColors.active;
    return Card(
      margin: EdgeInsets.zero,
      child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(row.number, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.active)),
            const SizedBox(height: 3),
            Text(row.customer, maxLines: 1, overflow: TextOverflow.ellipsis),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: statusColor.withValues(alpha: .1), borderRadius: BorderRadius.circular(99)), child: Text(status, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: statusColor))),
          PopupMenuButton<String>(
            tooltip: 'Invoice actions',
            onSelected: onAction,
            itemBuilder: (_) => [
              if (status != 'Paid') const PopupMenuItem(value: 'paid', child: ListTile(leading: Icon(Icons.payments_outlined), title: Text('Record Paid'), contentPadding: EdgeInsets.zero)),
              ...const [
                PopupMenuItem(value: 'Edit', child: ListTile(leading: Icon(Icons.edit_outlined), title: Text('Edit'), contentPadding: EdgeInsets.zero)),
                PopupMenuItem(value: 'Share', child: ListTile(leading: Icon(Icons.share_outlined), title: Text('Share'), contentPadding: EdgeInsets.zero)),
                PopupMenuItem(value: 'Duplicate', child: ListTile(leading: Icon(Icons.copy_outlined), title: Text('Duplicate'), contentPadding: EdgeInsets.zero)),
                PopupMenuItem(value: 'Delete', child: ListTile(leading: Icon(Icons.delete_outline), title: Text('Delete'), contentPadding: EdgeInsets.zero)),
              ],
            ],
          ),
        ]),
        const Divider(height: 24),
        Row(children: [
          Expanded(child: _InvoiceDetail('Invoice date', DateFormat('dd MMM yyyy').format(row.date))),
          Expanded(child: _InvoiceDetail('Due date', row.dueDate == null ? '—' : DateFormat('dd MMM yyyy').format(row.dueDate!))),
        ]),
        const SizedBox(height: 12),
        if (row.referenceNumber.isNotEmpty) ...[_InvoiceDetail('Order number', row.referenceNumber), const SizedBox(height: 12)],
        Row(children: [
          Expanded(child: _InvoiceDetail('Invoice amount', money.format(row.amount))),
          Expanded(child: _InvoiceDetail('Balance due', money.format(balance), strong: true)),
        ]),
        const SizedBox(height: 12),
        const Row(children: [Expanded(child: _InvoiceDetail('Currency', 'INR')), Expanded(child: _InvoiceDetail('Salesperson', '—'))]),
      ])),
    );
  }
}
...
class _InvoiceDetail extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 3),
    Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: strong ? FontWeight.w700 : FontWeight.w500)),
  ]);
}

class _InvoiceEmptyState extends StatelessWidget {
  const _InvoiceEmptyState();
  @override Widget build(BuildContext context) => const Center(child: Padding(padding: EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [
    Icon(Icons.receipt_long_outlined, size: 54, color: AppColors.textSecondary),
    SizedBox(height: 14),
    Text('No invoices found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
    SizedBox(height: 6),
    Text('Try changing your search or filters.', textAlign: TextAlign.center, style: TextStyle(color: AppColors.textSecondary)),
  ])));
}
